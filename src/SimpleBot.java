...
public class SimpleBot {
    ...
    //Шаблоны для анализа введенных данных (запрос:ключ к ответу)
    ...
    public String sayInReturn(String msg, boolean ai){
        String say = (msg.trim().endsWith("?"))?                            //Заканчивется ли введенная строка знаком "?"
                ELUSIVE_ANSWERS[random.nextInt(ELUSIVE_ANSWERS.length)]:    //Если да - то выбираем из ответов
                COMMON_PHRASES[random.nextInt(COMMON_PHRASES.length)];      //Если нет - то из общих фраз
        if (ai) {
            String message =
